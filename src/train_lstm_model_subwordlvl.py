...
- Uses SentencePiece subword tokenization (or falls back to simple word tokenization).
...
- Supports CLI flags to run training only, eval only, or both.
""" 
...
class DataPreprocessor:
    ...
    def trainSentencePiece(self, corpus_texts: List[str], model_prefix: str, vocab_size_local: int) -> Tuple[str, str]:
        ...
        os.remove(tmp_corpus)  # cleanup corpus file
...
class Seq2SeqModel:
    ...
    def build_inference_models_from_trained(trained_model_path: str, latent_dim_local: int, direction_suffix: str):
        ...
        encoder_inputs = full.input[0]
        # coz encoder_bilstm is a wrapper, calling encoder_bilstm(encoder_embedding(encoder_inputs)) returns the full outputs/states
        enc_emb = encoder_embedding(encoder_inputs)
        encoder_outputs, f_h, f_c, b_h, b_c = encoder_bilstm(enc_emb)  # returns list: output, f_h, f_c, b_h, b_c 
        state_h = keras.layers.Concatenate()([f_h, b_h])
        state_c = keras.layers.Concatenate()([f_c, b_c])
        encoder_model = keras.Model(encoder_inputs, [encoder_outputs, state_h, state_c], name=f"encoder_infer_{direction_suffix}")
    ...
    def compile_and_train(model, encoder_input, decoder_input, decoder_target,
                          save_prefix: str, batch_size_local: int, epochs_local: int, callbacks=None):
    ...
    @staticmethod
    def greedy_decode_sequence(encoder_model, decoder_model, input_seq, token_tool, max_dec_len, direction, use_sp=True) -> str:
        """greedy stepwise decoding with the inference encoder_model & decoder_model, returns decoded string (tokens to text)
        token_tool is ("sentencepiece", model_path) or ("keras_tokenizer", tokenizer_obj)"""
    ...
    def evaluate_on_testset(encoder_model, decoder_model, encoder_inputs, input_texts, target_texts, token_tool, max_enc_len, max_dec_len, direction, sample_limit=None):
        ...
        token_tool: ("sentencepiece", model_path) or ("keras_tokenizer", tok)
        """
        ...
        for i in range(N):
            ...
            pred_text = Seq2SeqModel.greedy_decode_sequence(encoder_model, decoder_model, enc_seq, token_tool, max_dec_len, direction, use_sp=(token_tool[0] == "sentencepiece"))
...
def run_pipeline(direction: str = "eng2deu",
    ...
    """params pretty self-explanatory no?"""
    ...
    token_data = preprocessor.prepareTokenData(input_texts, target_texts, direction, vocab_size_local, max_samples=samples)
    encoder_input = token_data["encoder_input"]
    decoder_input = token_data["decoder_input"]
    decoder_target = token_data["decoder_target"]
    ...
    # logger.info("Saved token tool info to %s", token_info_file)

    # split train/test indices
    train_idx, test_idx = Evaluator.train_test_split_indices(encoder_input.shape[0], test_ratio=0.2, seed=42)
    encoder_train = encoder_input[train_idx]
    decoder_input_train = decoder_input[train_idx]
    decoder_target_train = decoder_target[train_idx]
    encoder_test = encoder_input[test_idx]
    input_texts_test = [input_texts[i] for i in test_idx]
    ...
    # if we're doin eval_only, we load existing trained model and run evaluation
    ...
    if eval_only:
        ...
        metrics, results = Seq2SeqModel.evaluate_on_testset(encoder_model, decoder_model, encoder_test, input_texts_test, target_texts_test,
    ...
    model, components = Seq2SeqModel.build_token_seq2seq(input_vocab_size, target_vocab_size,
                                                        ...
                                                        dropout_local=dropout,
                                                        recurrent_dropout_local=recurrent_dropout)
    ...
    # train
    history, save_path = Seq2SeqModel.compile_and_train(model, encoder_train, decoder_input_train, decoder_target_train,
                                                       save_prefix=direction, batch_size_local=batch_size, epochs_local=epochs, callbacks=fit_callbacks)

    # from full model we build inference models..
    encoder_model, decoder_model = Seq2SeqModel.build_inference_models_from_trained(save_path, latent_dim, direction)
    ...
    # if we do train_only, we simply skip the long eval
    if train_only:
...
if __name__ == "__main__":
    ...
    if args.eval_only:
        result = run_pipeline(direction=args.direction, data_path=args.data_path, samples=args.samples,
